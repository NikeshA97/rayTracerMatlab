function col = illuminationModel_V7(A,d,hit,tmin,indx,o,v,cdpth,InOut)
%The inputs
...
%         newAout = TT*newAout';
%         newAout = SS*newAout;
%         phi = atan(newAout(1)/newAout(3));
%         theta = acos(newAout(2));
%         uu = phi/(2*pi);
%         vv = 1 - theta/pi;
%         
%         if uu<0
...
%         if v.texmap(vv+1,uu+1) == 0
%             otmp.Col = [0 0 0];
%         else
%             otmp.Col = [1 1 1];
%         end
          
% otmp.Col = shiftdim(v.texmap(uu+1,vv+1,:),1);
    ...
    otherwise % I only have spheres and triangles
        otmp = o.tri{indx};
        [Ain,Aout,drfl,dref,dn] = nextRayTriV2(A,d,tmin,otmp.vertex,otmp.kt(2),InOut);
        if mod(sum(round(Aout/5)),2) == 0 && otmp.spc == 0.001
                    otmp.Col = [1 1 0];
        end
        %
        IaKaOd = v.Ia * otmp.amb * otmp.Col;
